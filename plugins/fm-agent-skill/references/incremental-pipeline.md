...
9. `bug_validation`: dispatch `fm-bug-validate-worker` only for selected direct `MISMATCH` candidates. Use `probe_runner.py detect` once, then `probe_runner.py run --attempt <job-attempt>` for each immutable isolated attempt under `fm_agent_skill/probes/<bug-id>/attempt_<n>/`; then overwrite the current `fm_agent/bug_validation/summary.json`. Incremental cleanup removes any older summary before this phase.
