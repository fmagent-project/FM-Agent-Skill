...
7. `update_specs`: restore only hash-compatible sidecars, regenerate affected contracts, reconcile caller `info.json` expectations, and write native `fm_agent/incremental_updated_specs.json`. Retain the current run's module/file selection and `spec_update_*` records in `fm_agent/`.
