...
| verification | ready functions | schema-v2 result per current function/snapshot; MATCH/MISMATCH require structured A→B reasoning, high-confidence B, and MISMATCH additionally requires a concrete counterexample plus exact source quote; direct mismatch separated from dependency risk | Worker records a valid per-function `ERROR` for reasoning failure; malformed output retries and becomes a terminal job failure after its limit |
