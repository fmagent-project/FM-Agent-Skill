...
4. `extraction`: run `executor.py extract --project "$PROJECT"` (plus every selected `--submodule`). It writes immutable function copies and the plugin control inventory without calling FM-Agent.
