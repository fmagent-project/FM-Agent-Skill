...
8. `bug_validation`: run `job_planner.py --phase bug_validation` once, then use `durable_executor.py next` continuously to claim every registered direct-`MISMATCH` `fm-bug-validate-worker` job. Up to four independent jobs may run at once by default, subject to host capacity. In default `agent-executed` mode, do not call `probe_runner.py detect`: every registered language uses the Worker-selected `host-project-toolchain` execution pass. The Worker records a real public-entry probe using only an attempt-local workspace and cache, then its finalization pass consumes that immutable evidence. It selects the project's actual command, including Makefile projects; a missing SDK, device, compiler, cache, or public entry produces `unsupported/inconclusive` only after the Worker records that evidence. In optional `adapter` mode only, the Coordinator may run `probe_runner.py detect`, `probe_runner.py run`, and `reproduction_runner.py run`; C/C++ CMake adaptation additionally requires `CMakeLists.txt`, not merely a `cmake` executable. A build/syntax result cannot confirm a defect. An execution error requeues the same job. Write reports and a current summary only after every Bug Validator job has scheduler-successfully completed.
