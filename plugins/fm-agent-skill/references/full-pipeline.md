...
4. `extraction`: when CodeGraph is selected, first run `codegraph.py export --output "$PROJECT/fm_agent_skill/control/codegraph_export.json"`, then run `executor.py extract --project "$PROJECT" --codegraph-export "$PROJECT/fm_agent_skill/control/codegraph_export.json"` (plus every selected `--submodule`). Otherwise omit that option. Extraction uses mapped CodeGraph spans first, Tree-sitter when installed, then the language-specific fallback. It writes immutable function copies and the `fm_agent_skill` control inventory without calling FM-Agent. Never analyze test paths, emit a pseudo-function for declaration-only headers, hash-name artifacts, or write inline `[SPEC]`/`[INFO]` into copied source.
...
8. `bug_validation`: for every direct `MISMATCH`, start one `fm-bug-validate-worker` job and run its preparation pass. It writes a reviewed public-entrypoint probe contract in its immutable attempt directory. The Coordinator may run `probe_runner.py run` for build evidence, then must run `reproduction_runner.py run` for behavior evidence before invoking the same worker's finalization pass. A build/syntax result cannot confirm a defect. An execution error requeues the same job; unsupported dynamic adapters produce `inconclusive`. Write reports and a current summary only when direct candidates exist.
