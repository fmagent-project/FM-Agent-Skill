...
6. `specification`: after knowledge materialization, run `job_planner.py --phase specification` once, then `semantic_executor.py prepare`. It registers domain context plus every function across all phases/layers before execution, using caller-first dependencies and batches of up to eight by default. Dispatch only exact tickets for `fm-domain-context-worker` and `fm-spec-batch-worker`; never generate a Workflow script. Generated context is never normative evidence. Same-layer batches may run concurrently; each writes closed-schema-v2 `.spec.json`/`.info.json` sidecars with exact source quotes and separate normative/observed facts without changing source copies.
7. `verification`: run `job_planner.py --phase verification` once, then `semantic_executor.py prepare`. It writes validated `INCONCLUSIVE` results for low-confidence, observation-only specifications without launching a Worker. Dispatch one exact `fm-verify-function-worker` ticket only for each remaining high-confidence function. The Worker derives actual postcondition A and performs FM-Agent's structured A→B check against the externally grounded specification B. Mark a direct local violation `MISMATCH` only with a concrete counterexample and exact source quote; mark callers affected only through an invalid callee `DEPENDENCY_RISK`, not `MISMATCH`. Record a reasoning/tool failure as a well-formed `ERROR`; retry a malformed result as an output failure and never treat it as semantic evidence.
