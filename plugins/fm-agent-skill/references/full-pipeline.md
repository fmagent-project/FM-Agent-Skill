...
6. `specification`: first materialize `system_prompt.md`, the engine overview, one `phase_XX_types.txt` per phase, and the phase batch manifests. Process each phase caller-first, respecting inter-phase dependencies, and write schema-valid `.spec.json`/`.info.json` sidecars without changing extracted source copies.
...
8. `bug_validation`: send only direct `MISMATCH` results to probes/validation. For a CMake project, first use `probe_build.py` to configure and build in a run- and bug-specific directory under `fm_agent_skill/probes/`; never reuse the project's `build/` cache. Write reports and a summary only when there are direct candidates.
