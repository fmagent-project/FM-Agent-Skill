...
8. `bug_validation`: dispatch one `fm-bug-validate-worker` for each direct `MISMATCH`. First run `probe_runner.py detect`, then run each job with `probe_runner.py run --attempt <job-attempt>` in its immutable bug-specific attempt directory. The runner selects a safe build adapter; unsupported CUDA/ArkTS tooling yields `inconclusive`, never an arbitrary shell command. Write reports and a summary only when there are direct candidates.
